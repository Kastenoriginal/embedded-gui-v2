...
public class RaspberryLayout implements EmbeddedLayout {
    ...
    private void disableGridElements(ArrayList<Button> buttons, ArrayList<ComboBox<String>> pinTypeComboBoxes,
        ...
        for (int i = 0; i < PIN_COUNT; i++) {
            ...
            if (RaspberryHashMap.PWR5.equals(selectedItem) ||
                    RaspberryHashMap.PWR3.equals(selectedItem) ||
                    RaspberryHashMap.GND.equals(selectedItem) ||
                    RaspberryHashMap.EEPROM.equals(selectedItem)) {
    ...
    private void enableRowElements(int checkBoxId) {
        ...
        for (ComboBox<String> comboBox : pinTypeComboBoxes) {
            ...
            if (Pin.PIN_TYPE_I2C.equals(currentComboBoxSelectedItem)) {
    ...
    private void synchronizeI2cElements(ComboBox<String> pinTypeComboBox) {
        ...
        } else if (RaspberryHashMap.GPIO.equals(pinTypeSelectedItem) && pinTypeComboBox.getItems().contains(Pin.PIN_TYPE_I2C)) {
            for (ComboBox<String> comboBox : pinTypeComboBoxes) {
                if (comboBox.getItems().contains(Pin.PIN_TYPE_I2C)) {
                    comboBox.getSelectionModel().select(RaspberryHashMap.GPIO);
                }
            }
        }
    }
